...
## reading the train data for X(features) and Y (class of activities), againe i named the columns in X_Train "feature_names" from the previous dataset features and columns in Y_Train to code)
X_Train<- read.table("UCI HAR Dataset/train/X_train.txt", col.names = features$feature_names)
Y_Train<- read.table("UCI HAR Dataset/train/Y_train.txt",col.names = "code")
## reading the test data:the above mentioned process is repeated for test data set as well
X_Test<- read.table("UCI HAR Dataset/test/X_test.txt", col.names = features$feature_names)
Y_Test<- read.table("UCI HAR Dataset/test/Y_test.txt", col.names = "code")
# reading the subjects for both test and train dataset, i called the column names to subject, this column contains numbers from 1 to 30 that is the coded version of who performed those activities
...
# the final merged data set is combined by rows from Subject, Y, X datasets.
Merged_Data <- cbind(Subject, Y, X)

# part2:select function keeps only the variables we mention after the first argumnet which is subject and subsets the merged_Data dataset to extract only the measurements on the mean and standard deviation and the i assigned it to Tidy_Data .
...
# code column (second column) in Tidy_Data renamed into activities
